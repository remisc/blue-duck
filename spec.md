...
| F4 | User selects the **total engagements** — how many shots appear in the sequence (can exceed distinct target count, allowing repeats) |
| F5 | Each sequence must be randomly shuffled; the same label may appear multiple times |
...
| F15 | Total engagements input (minimum 1) |
